<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="会员购买记录ID" prop="membershipLevel">
        <el-input
          v-model="queryParams.membershipLevel"
          placeholder="请输入会员购买记录ID"
          clearable
          size="small"
    ...
    <el-table v-loading="loading" :data="recordList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="会员购买记录ID" align="center" prop="memberPurchaseRecordId" />
      ...
      <el-table-column label="支付类型: 微信、支付宝、网银" align="center" prop="payType" />
...
<script>
import { listRecord, getRecord, delRecord, addRecord, updateRecord } from "@/api/fee/memberCardRecord";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.memberPurchaseRecordId)
    ...
    handleUpdate(row) {
      this.reset();
      const memberPurchaseRecordId = row.memberPurchaseRecordId || this.ids
      getRecord(memberPurchaseRecordId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.memberPurchaseRecordId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const memberPurchaseRecordIds = row.memberPurchaseRecordId || this.ids;
      this.$modal.confirm('是否确认删除会员购买记录编号为"' + memberPurchaseRecordIds + '"的数据项？').then(function() {
        return delRecord(memberPurchaseRecordIds);
      }).then(() => {
        this.getList();
